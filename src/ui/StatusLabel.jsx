...
function StatusLabel({ status, labelName = "" }) {
    let statusStyle;
    ...
    return <span className={`px-2 py-0.5 rounded-md text-xs capitalize whitespace-nowrap ${statusStyle}`}>{status?.replaceAll("-", " ") || labelName?.replaceAll("-", " ")}</span>
}
