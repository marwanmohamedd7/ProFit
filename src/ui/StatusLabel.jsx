...
function StatusLabel({ status, customStyle = "", labelName = "" }) {
    ...
    if (status?.toLowerCase() === "cancelled" || status?.toLowerCase() === "rejected" || status?.toLowerCase() === "blocked") statusStyle = isDarkMode ? `text-red-500 bg-red-900 bg-opacity-50` : `text-red-500 bg-red-100`;
