...
function Header() {
  ...
  return (
    <nav className="flex items-center justify-between border-solid border-b border-grey-100 px-3 py-0.5">
      <button onClick={() => setIsOpen(value => !value)} className="ml-[-1.55rem] text-sm bg-gray-50 border border-gray-300 rounded-full p-1 text-gray-500 z-10">{isOpen ? <TbArrowBarToLeft /> : <TbArrowBarToRight />}</button>
      <div className="flex justify-end gap-4 divide-x divide-gray-200 items-center p-2.5 row-span-1">
