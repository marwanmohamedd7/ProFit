...
function Button({ children, onClick, type, to, name: pageName, customeStyle, disabled = false }) {
  ...
  const btnStyles = {
    ...
    primary: `flex items-center justify-center gap-2 ${isDarkMode ? 'bg-blue-900 text-white' : 'bg-blue-700 text-white'} transition-all duration-300 border border-transparent font-bold text-sm tracking-wide py-2 px-4 rounded-md ${isDarkMode ? 'hover:bg-blue-800' : 'hover:bg-blue-600'} focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-blue-600`,
    secondary: `flex justify-center items-center gap-1.5 transition-all duration-300 px-4 py-2 border text-sm font-bold rounded-md ${isDarkMode ? 'border-gray-600 text-gray-300 hover:bg-gray-800 hover:text-white' : 'border-blue-700 text-blue-700 bg-white hover:bg-gray-100'} focus:outline-none focus:ring-2 focus:ring-offset-1 focus:ring-blue-600`,
