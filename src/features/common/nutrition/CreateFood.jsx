...
function CreateFood({ onCloseModal }) {
    const { handleSubmit, register, formState: { errors }, control, watch } = useForm()
    return (
        ...
        <form className="flex flex-col capitalize divide-y" onSubmit={(e) => e.preventDefault()}>
            <div className="py-4 space-y-2">
                <UploadImageForm
                    ...
                    // src={transformationValues?.image ?? null}
                    rules={{ required: "food photo is required" }}
                />
                {/* <img className="w-24 h-auto rounded-md" src="/uifaces-popular-image (1).jpg" alt="" /> */}
                <div className="flex justify-center gap-2">
                    <InputFloatingLabel item={{ id: "foodName", label: "Food Name", value: watch("foodName") }}
                        // disabled={isLoading}
