...
function MealFood({ food, section, disabled, isExist = false }) {
    ...
    return (
        <div className={`p-4 rounded-md w-full border ${inadvisableFood ? (isDarkMode ? `${colors.bg_red_900} ${colors.border_slate_700} bg-opacity-60` : `${colors.bg_red_100}`) : `${isDarkMode ? `${colors.bg_slate_800} ${colors.border_gray_700}` : colors.bg_gray_50}`}`}>
                    ...
                    <div className="flex items-center gap-2 xl:grow-0 grow">
                        ...
                        <Modal>
                            <Modal.Open opens="delete-food">
                                <button disabled={disabled} className={`${isDarkMode ? `${colors.bg_red_800} ${colors.text_white} hover:${colors.bg_red_700}` : `${colors.bg_red_700} ${colors.text_white} hover:${colors.bg_red_600}`} transition-all duration-300 xl:p-3 p-2.5 rounded-lg text-lg disabled:cursor-not-allowed`}><BiTrash /></button>
                            </Modal.Open>
