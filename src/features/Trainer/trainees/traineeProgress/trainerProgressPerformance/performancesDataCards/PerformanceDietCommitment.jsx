...
function PerformanceDietCommitment({ data }) {
    ...
    const { percentage, totalDaysMacros, totalEatenDaysMacros, dietDetails } = data ?? {};
    ...
    return (
        <TrainerProgressPerformanceCards detailedData={{ chart: "biBarChart", data: dataDetails ?? [] }} icon={<AppleIcon fill={true} />} color={isDarkMode ? `text-green-500` : `text-green-600`} title="diet commitment">
            ...
            <span className={`text-sm text-right ${isDarkMode ? colors.text_gray_50 : colors.text_gray_700}`}>{totalEatenDaysMacros || data?.totalEatenDayMacros} / {totalDaysMacros || data?.totalDayMacros} <strong>Kcal</strong></span>
